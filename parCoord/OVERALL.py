...
# url MOOCVIZ edit //http://128.52.161.241:3000/new_viz_step_6?viz_id=3&offering_id=7

#url connecting GITHUB http://128.52.161.241:3000/system/visualizations/chelsea-iveel/moocviz/
import csv

OVERALL_INPUT = '../Overall.csv'
INPUTFILENAMES = [
...
FIELDS_NAME = ['viewed', 'explored', 'certified', 'grade', 'events', 'active days', 'video play', 'chapters', 'forum posts']
#[fi+'_avg' for fi in FIELDS]

COURSE = 'course_id'

#for finding overall average
OVERALL_COUNT = {}
OVERALL_SUM = {}
OVERALL_AVE = {}
for fi in FIELDS:
...
##Compute average for overall course
for fi in FIELDS:
    if OVERALL_COUNT[fi]>0:
        OVERALL_AVE[fi] = round(1.0*OVERALL_SUM[fi]/OVERALL_COUNT[fi], 2)
        
for inputfilename in INPUTFILENAMES:
    outputfilename = OUTPUTFOLDER + '/' + inputfilename.split('/')[-1]
    with open(outputfilename, 'a') as csv_out:
        csvwriter = csv.writer(csv_out, delimiter=DELIMITER, quotechar=QUOTECHAR, quoting=csv.QUOTE_MINIMAL)
        csvwriter.writerow(["overall"]+[OVERALL_AVE[fi] for fi in FIELDS])
